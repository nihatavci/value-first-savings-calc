    ...
    <div className="w-full max-w-4xl mx-auto my-8 px-4">
      <Card className="border-brand-200 shadow-lg">
        <CardHeader className="text-center border-b pb-6">
          <CardTitle className="text-3xl font-bold text-brand-800">Savings Optimization Calculator</CardTitle>
          ...
          <div className="mb-8">
            <Tabs value={steps[activeStep]} className="w-full">
              <TabsList className="grid w-full grid-cols-3 h-auto">
                <TabsTrigger 
                  value="pain-point" 
                  className="py-3 data-[state=active]:bg-brand-100 data-[state=active]:text-brand-800"
                  disabled
                >
                  <span className="flex items-center">
                    <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full mr-2 ${activeStep >= 0 ? 'bg-brand-600 text-white' : 'bg-gray-200 text-gray-600'}`}>1</span>
                    Pain Point
                  </span>
                </TabsTrigger>
                <TabsTrigger 
                ...
                >
                  <span className="flex items-center">
                    <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full mr-2 ${activeStep >= 1 ? 'bg-brand-600 text-white' : 'bg-gray-200 text-gray-600'}`}>2</span>
                    Quantify
                  </span>
                </TabsTrigger>
                <TabsTrigger 
                ...
                >
                  <span className="flex items-center">
                    <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full mr-2 ${activeStep >= 2 ? 'bg-brand-600 text-white' : 'bg-gray-200 text-gray-600'}`}>3</span>
                    Proposal
                  </span>
                </TabsTrigger>
              </TabsList>
          ...
          <div className="absolute top-6 right-6 flex space-x-1">
            {(Object.keys(CURRENCY_SYMBOLS) as Currency[]).map((curr) => (
              <Button 
                ...
                className={currency === curr ? "bg-brand-600 hover:bg-brand-700" : ""}
        ...
        <CardFooter className="flex justify-between border-t pt-6">
          ...
          <Button 
            ...
            className={`flex items-center ${activeStep < steps.length - 1 ? 'bg-brand-600 hover:bg-brand-700' : 'hidden'}`}
          >
